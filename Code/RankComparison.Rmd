...
The tests below are very similar to the ones above, the only difference being
grouping by Year (2 if 2022 and 3 if 2023).
```{r}
set.seed(1)
...
The following code could be used to see what happens if we remove prelims. 
The sample sizes in the test become very small: 26 observations and 9 clusters.
```{r}
#nations_noprelim <- read.csv("../Data/Clusrank2019vs2022NoPrelims.csv")
