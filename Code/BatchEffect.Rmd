...
```{r}
nested <- glmer(RxnTime ~ (1|Year) + (1|Batch), data=men_pooled, family = Gamma(link="log"))
...
```{r}
old_finals_two_effect <- glmer(RxnTime ~ (1 | Year) + (1 | Batch), data = old_finals, family = Gamma(link = "log"))
men_finals_two_effect<- glmer(RxnTime ~ (1 | Year) + (1 | Batch), data = men_finals, family = Gamma(link = "log"))
old_pooled_two_effect <- glmer(RxnTime ~ (1 | Year) + (1 | Batch), data = old_pooled, family = Gamma(link = "log"))
...
simulation <- function(model, base, RxnTime) {
  simfit <- function(model, n = 10000000) {
    ...
    shape <- 1 / sqrt(sd_year^2 + sd_year_batch^2)
...
simulation(old_finals_two_effect, old_finals_one_gamma_log, 0.1)
simulation(men_finals_two_effect, men_finals_one_gamma_log, 0.1)
simulation(old_pooled_two_effect, old_pooled_one_gamma_log, 0.1)
simulation(men_pooled_two_effect, men_pooled_one_gamma_log, 0.1)
```

```{}
All probabilities are for RxnTime=0.1 seconds
old finals: 0.0452759 vs 0.0004389
men_finals: 0.0647663 vs 0.000856
old pooled: 0.0779887 vs 0.0057613
men_pooled: 0.0897533 vs 0.0068649
