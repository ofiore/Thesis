...
The code below loads in the dataset from the csv file.
```{r}
#1
times <- read.csv("../Data/Thesis2023.csv")
colnames(times) <- c("Year", "Type", "Time", "RxnTime", "Gender", "Heat")
...
```{r}
summary(men_pooled_year_gamma_log)
summary(men_old_pooled_year_gamma_log)
```
...
#4
library(lme4)
men_old_pooled_heat_gamma_log <-glmer(RxnTime ~ (1 | Heat), data = men_old_pooled, 
                             family = Gamma(link = "log"))
men_pooled_heat_gamma_log <-glmer(RxnTime ~ (1 | Heat), data = men_pooled, 
                         family = Gamma(link = "log"))
```

```{r}
summary(men_pooled_heat_gamma_log)
```


This creates the models with a year effect and heat effect.
```{r}
#5
library(lme4)
men_old_finals_two_effect_gamma_log <- glmer(RxnTime ~ (1 | Year) + (1 | heat), 
                                   data = men_old_finals, family = Gamma(link = "log"))
men_finals_two_effect_gamma_log<- glmer(RxnTime ~ (1 | Year) + (1 | heat), 
                              data = men_finals, family = Gamma(link = "log"))
men_old_pooled_two_effect_gamma_log <- glmer(RxnTime ~ (1 | Year) + (1 | heat), 
                                   data = men_old_pooled, family = Gamma(link = "log"))
men_pooled_two_effect_gamma_log <- glmer(RxnTime ~ (1 | Year) + (1 | heat), 
                               data = men_pooled, family = Gamma(link = "log"))
```
```{r}
summary(men_pooled_two_effect_gamma_log)
summary(men_old_pooled_two_effect_gamma_log)
```
...
```{r}
anova(men_pooled_heat_gamma_log, men_pooled_two_effect_gamma_log)
```
...
sim_two_effect_time <- function(model, base, RxnTime) {
  simfit <- function(model, n = 10000000) {
    ...
    sd_year <- vc$sdcor[1]
    sd_year_heat <- vc$sdcor[2]
    
    z1 <- rnorm(n, sd = sd_year)
    z2 <- rnorm(n, sd = sd_year_heat)
...
```{r}
set.seed(1)
sim_two_effect_time(men_old_pooled_two_effect_gamma_log, men_old_pooled_one_gamma_log, 0.09)
sim_two_effect_time(men_pooled_two_effect_gamma_log, men_pooled_one_gamma_log, 0.09)
```
...
```{r}
#13
sim_one_effect_time(men_old_finals_heat_gamma_log, men_old_finals_one_gamma_log, 0.1)
sim_one_effect_time(men_finals_heat_gamma_log, men_finals_one_gamma_log, 0.1)
sim_one_effect_time(men_old_pooled_heat_gamma_log, men_old_pooled_one_gamma_log, 0.1)
sim_one_effect_time(men_pooled_heat_gamma_log, men_pooled_one_gamma_log, 0.1)
```
