...
\jy{Owen, does this include both dash and hurdle? Could you do a
  pooled analysis since there is no gender difference and event
  differece? Need to summarize how many athletes are included in each
  analysis.}
...
significantly reduced the number of athletes and clusters, narrowing the 
scope of the analysis. Each athlete can be considered as a cluster, with
reaction times from the `treatment' group (2022 World Championships) and 
the `control' group (national competition) within the cluster. Cluster 
sizes ranged from three to six, with a median size of four. This
setting forms the basis for rank-based comparison where the
within-cluster dependence needs to be accounted for.
