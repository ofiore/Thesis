...
\subsection{World Championships 1999--2022}\label{sec:dataworld}
...
and one
athlete who did not compete.  We also consider data that
excludes 2022 to see how our analysis differs based on the exclusion of one
particular year of interest.


The data under consideration is summarized using a boxplots in 
Figure~\ref{fig:Boxplot}.  It is clear that the reaction times in the 
...
\cite{brosnan2017effects} found median reaction times of 0.156 seconds 
for men when looking at data from 1999 to 2014.
We can also see from Figure~\ref{fig:Boxplot} that the reaction times clearly
...
\of{I don't know how to say this}. \eds{is my change correct?} Each athlete was 
...
within each cluster label times to be from either the 2019 World Championships
...
%\eds{check these numbers? 0.099?}\of{resolved}
...
and heat effect was the best model, as indicated by an ANOVA test 
\eds{I think it may be better to call this a likelihood ratio test rather than
an ANOVA test, assuming that is the formal test being used} and decrease in
AIC.  For the entire pooled dataset, the AIC for the venue effect only model was
$-1847.9$, the AIC for the heat effect only model $-1894.4$, and the AIC for the 
two effect model, which we will present throughout this section, was $-1898.9$. 
Thus, the two
...
two thresholds: 0.1 and 0.08. When examining the All Pooled dataset, a probability
...
Table~\ref{tab:Sim_time} shows that the inclusion of 2022 results in a lower
...
%\of{Should I report table~\ref{tab:EffectComparison}}
%\begin{table}
%  \centering
...
produced stronger \eds{lower?} p values than the 2019 versus 2022 comparison, with the
