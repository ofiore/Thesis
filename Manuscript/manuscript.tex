...
The notable faster RTs at the 2022 World Championships
...
were systematically faster by comparing them against (1) national
competitions in 2022 and (2) RTs from the 2019 and 2023 World
Championships to evaluate deviations from historical norms, using data
from athletes who competed in both events in the comparison.
...
to be significantly different. This exclusion is expected, as reaction
time plays a smaller role in longer sprint events, where acceleration
off the blocks is less decisive.
...
reacted differently across events.
...
this setting, we have clustered data with subunit grouping. Each
athlete serves as a cluster, with multiple RTs recorded from the same
athlete across different competitions.
...
criterion for disqualification in elite sprinting. Using historical RT data from 
...
alternative threshold would better align with observed reaction time 
distributions.
