...
Table~\ref{tab:Gamma_parameters}, with both including and excluding 2022 data.
Three models were considered, differing in there random effects: venue effect
...
changes to the estimates of other parameters
...
Both plots agree to some extent with the pattern of the centers of the box plots in
Figure~\ref{fig:Boxplot}. The mismatch is due to the venue effect, which has a
higher standard deviation than the venue effect. The venue effect for 2022 is
lower than that for any other years, but may not be as extreme as one would
...
the difference is not in a any big magtinude. For example, the probability for
seeing a reaction time below 0.10 seconds changes from $3.54\cdot 10^{-3}$ to
$4.20\cdot 10^{-3}$ when 2022 is included. They corresponding to roughly one in
