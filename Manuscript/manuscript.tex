...
The first objective of this paper is to investigate whether reaction
times at the 2022 World Championships were significantly different
from other competitions, focusing specifically on athletes who
competed in multiple events. By employing a matched-pairs design, we
compare reaction times from the same athletes across different
championships, including the 2022, 2019, and 2023 World Championships,
as well as the 2022 national-level competitions. This approach allows
us to isolate the effect of the competition year on reaction times
while controlling for individual athlete performance. Our analysis
provide a more formal statistical evaluation of the disqualification 
threshold, building upon the informal comparisons previously made 
by others.


There is an ongoing debate over the appropriateness of the 0.1-second
threshold for disqualification, with past studies suggesting both
lower and higher limits. \citet{pain2007sprint} examined 
...
The second objective of this paper is to further investigate the
appropriateness of the 0.1-second threshold by modeling reaction times
from athletes who competed in world championships from 1999 onward. We
employ a gamma linear mixed-effects model with two random effects: one
for the venue (year) and another for the heat nested within each
venue. This model allows us to estimate the probability of an
athlete’s reaction time falling below a specific threshold, such as
0.1 seconds. By examining reaction times across different years and
heats, we can assess the consistency of reaction times and whether the
current disqualification threshold is supported by statistical
evidence.
