...
in order to model the reaction time data. 
...
how the data were collected and begins to detail the generalized linear mixed-effect 
 model (GLMM) that is further developed in Section~\ref{sec:glmm}. The 
signed-rank test that analyzes reaction times for the same athletes across
different competitions is
also described in Section~\ref{sec:rank}.  Section~\ref{sec:Results}
...
roughly 0.1 to 0.18 seconds.} \eds{is it correct now?}
...
or are you trying to say something different here?}  The 2022 World
...
%consistency.}
%\eds{I think the graphs look good}
...
statistic.  \eds{simulate the null distribution of the test statistic?}
...
Table~\ref{tab:Clusrankresults} shows the results generated from four \eds{three?}
tests in R.
