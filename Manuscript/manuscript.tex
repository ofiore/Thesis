...
warnings when creating the finals only models
\of{Is this section okay? Please let me know what I should rework}
...
\begin{table}
  ...
  \begin{tabular}{c c c c c}
      \toprule
      Dataset & Batch SE & Year SE & Intercept & Residual SE \\
      \midrule
...
\begin{table}
  ...
  \begin{tabular}{c c c c c} 
   \toprule
   Comparison & $exact = True$ & $exact \neq True$ & Sample size & Clusters/Athletes \\ 
   \midrule
   2019 vs 2022 & $2.9\cdot10^{-3}$ & $2.3\cdot10{-2}$ & 65 & 15 \\ 
   Nat vs Inter & $2.2\cdot10^{-16}$ & $3.1\cdot10^{-4}$ & 61 & 15 \\
   \bottomrule
  \end{tabular}
...
We use a \texttt{clusWilcox.test()} and set the permutation number (B) to be
1000 as the sample size in this analysis is small.  The sample size indicates
the total number of observations while clusters represnts the number of athletes
in each dataset.  It is worth noting that it is entirely coincidental that there
are fiftenn athletes in sets.
