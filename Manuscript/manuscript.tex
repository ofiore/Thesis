...
disqualified RTs were included due to their low frequency. A
...
2022 World Championships compared to the 2022 national competitions. 
\eds{This does not appear true based on the data currently in Table 1. 
2022-2023 seem to have the lowest p-values.  Which is correct?}
For both men and women, permutation test results were highly significant,
...
the venue level $\tau_v$ on the log-scale of the $\mu$ parameter
and at the heat level $\tau_h$ on the log-scale of the $\sigma$
...
the standard deviation of the venue-level random-effect variance,
which increases from 0.043 to 0.058 . 
\eds{are you reporting estimated variances or SDs of the random effects here and 
in the Table?  In the table, we have $\tau_v$ and $\tau_h$ (SD) labels, not 
$\tau_v^2$ and $\tau_h^2$ (variances).  Please check and correct as needed.}  
These results
...
meets. 
Ideally, a centralized database containing RTs from
...
distinctions. \eds{is this still true in the updated analyses?}
...
\eds{when the supplement is complete, include an additional sentence here
that summarizes the additional analyses presented in the supplemental material}
