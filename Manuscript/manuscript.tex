...
\begin{abstract}
  ...
  analysis of the data.  Devon Allen had previously run the third-fastest time in history when 
  he ran 12.84 earlier in 2022 and was considered to be one of the favorites to win.  His 
  disqualification thus prompted questions into whether the timing equipment used at the World
  Track Championships was accurate and whether Allen actually reacted as fast as the 
  timing equipment stated he did. This paper questions the reaction time disqualification barrier, which is
  currently 0.1 seconds to determine whether this is a reasonable threshold as
  well as look at data that calls into question whether the timing equipment
  used at the 2022 Track and Field World Championships was faulty.
  There is little at this point that could be done to rectify any potential 
  mis-disqualification other than to try to prevent it from happening again.  
  This matter needs to be addressed because Devon Allen's disqualification could 
  be a problem that could continue to re-appear.  If athletes
  ...
  the minimum reaction time allowed.
...
with shock from both Allen and fans alike.  The crowd erupted with jeers when
they found out that Allen was disqualified, angry at the result.  Allen himself
appeared frustrated and angered with the result. Had he reacted just 0.001 
seconds slower he would have been allowed to compete and been spared waiting
another two years until the next World Track and Field Championship.
%Can talk about youtube video and put the link in text here

2) and 3)
Other scholarly papers have discussed track and field reaction times but not in the
context of whether the minimum reaction time barrier is too low or whether timing
systems were accurate.  Outside the blog posts from LetsRun.com, there have not been
any published papers regarding Devon Allen's disqualification in particular.  This is not
necessarily surprising given that his disqualification occurred only last August.
...
cited graphs, statistics such as median reaction time, and a reference to a study
published in cooperation with IAAF in 2009 questioning the validity of the 0.1 
second reaction barrier.  The paper analyzed sprinters reaction times
and the biological processes that went into reacting to the starting gun and
concluded that athletes can react in as low as 0.08 seconds. It even recommended
...
possibility of a false disqualification would be lowered \citep{Komi}.
...
%Should I include how I read LetsRun and talk about any personal experiences
%I have using it?
...
equipment rather than reacting illegally fast.  Currently the barrier is 0.10 seconds, any time
...
%This should go to data section
...
competition (national or international) was specified to be the group.  Next,
a Clustered Wilcoxon rank sum test was performed using the clusrank R package
\citep{Clusrank}.  Using a clustered rank sum test was chosen over a typical t 
test so as to avoid any assumptions about the distribution of population means.  
...
is preferable when the cluster sizes are informative \citep{Clusrank}.  We do not care that some
...
%This should go in the method section, generate from fitted model explain what
...
%results in the methods section).  Discussion should show that random effect
...
The p values produced by the clustered rank sum tests were 0.000155 for the
same athletes competing in the same year, and 0.001464 for the same athletes in
different years.  To put it simply, there is not a reasonable explanation for 
this.  
It is inconceivable that so many athletes would improve such a significant
