...
Now we make the density graph
```{r}
make_graph <- function(n = 1000000){
  plot(density(samplefit, n=n), xlim = c(0, 0.3), ylim = c(0, 25), xlab = "",
     main = "")
  curve(dGG(x, mu = exp(gg3b$mu.coefficients[1]),
          sigma = exp(gg3b$sigma.coefficients[1]),
          nu = gg3b$nu.coefficients[1]),0, 0.3, add = TRUE, col = "blue",
      main = "", xlab = "", ylab = "")
}
density_plot <- make_graph(1000000)
```

And combine it with the residuals
```{r}
library(gridExtra)
resid_plot <- qq_conf_plot(residuals(gg3b), dparams = list(mean = 0, sd = 1))
density_plot <- grid.arrange(resid_plot, density_plot, ncol = 2)
ggsave(filename = "ResidualsDensityPlot", path = "../Manuscript",
       plot=resid_density, height = 4, width = 8)
```
